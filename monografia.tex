...
\chapter{Avaliação da ferramenta} \label{ch:avaliacao}
    ...
    Nessa seção serão descritos os resultados obtidos com a ferramenta desenvolvida. Na subseção \ref{sc:apache} serão descritos a avaliação da ferramenta feita com a mineração dos repositórios de código aberto da \textit{Apache Software Foundation}. Na subseção \ref{sc:estudo-caso}, será demonstrada a reprodução de um estudo de mineração de software já publicado na academia, utilizando os dados processados pelo MetricMiner.

    \section{Resultados obtidos com a mineração de repositórios públicos} \label{sc:apache}

        Para avaliar a capacidade da ferramenta desenvolvida, foram utilizados os projetos de código aberto disponibilizados pela \textit{Apache Software Foundation}. O código desses projetos pode ser acessado por por meio do sistema de controle de versão Git, a lista completa de projetos se encontra em \url{http://git.apache.org/}. No total, foram cadastrados e processados 307 projetos de código aberto.
        ...
        A mineração desses repositórios foi utilizada para dirigir a implementação do MetricMiner. Diversas pequenas otimizações foram realizadas ao longo do desenvolvimento e seus resultados foram medidos com a mineração desses repositórios.


        \begin{table}\begin{center}
        \begin{tabular}{| p{4cm} | p{1.8cm} | p{1.8cm} | p{1.8cm} | p{1.8cm} | p{1.8cm} | }
            \hline                        
            \textbf{Data} & \textbf{07/08/12} & \textbf{10/08/12} & \textbf{16/08/12} & \textbf{07/10/12} 
             & \textbf{Versão atual} \\
            \hline
            \textbf{Tempo de duração médio das tarefas} & 5.3 & 5.8 & 4.4 & 5.0 & 3.0 \\
            \hline
            \textbf{Tempo médio das dez tarefas mais lentas} & 58.2 & 77.9 & 40.3 & 44.2 & 35.3 \\
            \hline
            \textbf{Tarefas com erros} & 0 & 3 & 0 & 12 & 0 \\
            \hline
        \end{tabular}
        \caption{Dez projetos com maior número de artefatos processados no MetricMiner \label{tab:artefatos}}
        \end{center}\end{table}
        ...
        \begin{itemize}
            \item \textbf{Utilização \codechunk{StatelessSession} do Hibernate:} A classe utilizada normalmente para se realizar consultas ao banco de dados com o Hibernate é a \codechunk{Session}. Por padrão, essa classe faz cache das entidades persistidas ou buscadas no banco de dados. Durante os testes, percebeu-se que esse comportamento padrão causava um consumo de memória muito alto. Para contornar esse problema, foi utilizada a classe \codechunk{StatelessSession}, que não realiza nenhum tipo de cache dos dados.
            \item \textbf{Consulta para buscar códigos fonte no cálculo de métricas:} Inicialmente, para buscar os códigos fonte para calcular as métricas de código na tarefa \codechunk{CalculateAllMetricsTask}, era utilizada uma consulta que envolvia uma operação \codechunk{JOIN} de SQL trazendo o conteúdo dos arquivos do banco de dados. Como a quantidade de resultados era muito grande, foi necessário paginar os resultados dessa consulta, e portanto essa era executada diversas vezes para processar todas as versões dos arquivos dos projetos. Com o aumento do número de entradas armazenadas, esse processo se tornou  lento. Para otimizar essa tarefa, ao invés de realizar diversos \codechunk{JOIN}'s, foi utilizada apenas uma consulta para trazer todas os identificadores dos arquivos de código para calcular as métricas e só então o conteúdo de fato desses arquivos eram trazidos do banco de dados, diretamente pela sua chave primária. Assim, ao invés de executar diversas operações de \codechunk{JOIN} (que é uma operação custosa em bancos de dados relacionais), é feita apenas uma consulta envolvendo tal operação.
