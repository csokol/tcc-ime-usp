...
\chapter{Avaliação da ferramenta} \label{ch:avaliacao}
    ...
    \section{Resultados obtidos com a mineração de repositórios públicos} \label{sc:apache}
        ...
        A mineração desses repositórios foi utilizada para dirigir a implementação do MetricMiner. Diversas pequenas otimizações foram realizadas ao longo do desenvolvimento e seus resultados foram medidos com a mineração desses repositórios. Dentre essas otimizações podemos destacar as seguintes:
        \begin{itemize}
...
query.setParameter("project_id", project.getId())
    ...
    .setMaxResults(PAGE_SIZE);
            ...
            \item \textbf{Cache de segundo nível:} Além do cache de primeiro nível (descrito brevemente no primeiro item), o Hibernate possui um segundo cache
        \end{itemize}
