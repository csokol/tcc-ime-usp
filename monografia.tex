...
\chapter{Introdução}
    ...
    de bugs \cite{Kagdi:2007}. Por meio da mineração desses dados, podem ser desenvolvidos
    encontradas diversas informações, como por exemplo a indentificação de classes que são 
    muito modificadas, classes mais propensas a falhas, entre outras. Com essas informações, 
    equipes de desenvolvimento podem tomar ações para aprimorar o processo de desenvolvimento 
    do sistema.

    dá alguns exemplos aqui, tipo se conseguirmos identificar classes que sao muito modificadas, mais propensas a bugs, podemos dar mais atencao a elas… se detectarmos trechos de codigo que somente um dev mexe, podemos trabalhar para compartilhar esse conhecimento, e etc.
...
%% Chico, não gostei muito dessa seção. Vc falou muitas coisas legais, mas pra quem não entende de MSR, ficou complicado. Gaste mais alguns parágrafos explicando o que é, qual o objetivo, e etc. 
%% Depois vc pode mandar bala e comentar classificação, tópicos estudados na área e etc.
%% Dá uma olhada no curso de Evolução de Software que eu e o Gustavo demos. Tem muita coisa introdutória que seria legal aparecer aqui!
    (revisar)
        ...
        pode estar armazenado em um sistema de controle de versão, até mensagens em listas de
...
\chapter{MetricMiner} \label{ch:arquitetura}
    Nessa seção é apresentada a ferramenta desenvolvida, o MetricMiner. Na seção \ref{sc:tecnologias}, as tecnologias utilizadas para o desenvolvimento do sistema serão descritos brevemente. Em \ref{sc:abordagem}, será apresentado o fluxo do processo de mineração realizado pelo MetricMiner e as principais funcionalidades implementadas. Em \ref{sc:arquitetura}, serão presentados os detalhes da arquitetura da ferramenta.
