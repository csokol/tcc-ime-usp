...
\chapter{Avaliação da ferramenta} \label{ch:avaliacao}
    ...
    \section{Resultados obtidos com a mineração de repositórios públicos} \label{sc:apache}
        ...
        Para avaliar a capacidade da ferramenta desenvolvida, foram utilizados os projetos de código aberto disponibilizados pela \textit{Apache Software Foundation}. O código desses projetos pode ser acessado por por meio do sistema de controle de versão Git, a lista completa de projetos se encontra em \link{http://git.apache.org/}. No total, foram cadastrados e processados 307 projetos de código aberto.

        Esse conjunto de projetos minerados possui grande variedade. Em relação ao domínio de aplicação, existem projetos de domínios complexos como o \textit{Apache Harmony} - uma implementação da JVM - como outros mais simples como \textit{Apache Commons Exec} - uma biblioteca java para a execução de comandos do sistema. Além disso os projetos variam bastante em relação ao tamanho, a tabela 
\begin{verbatim}
\end{verbatim}
...
git show 62d7d27bb005ac0f58d5b6a8d2a161943315eded --pretty=format:%H author %ae date %ai message %s