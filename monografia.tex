...
\chapter{Arquitetura do MetricMiner} \label{ch:arquitetura}
    ...
    \section{Decisões arquiteturais} \label{sc:arquitetura}
        ...
        \subsection*{Fila de execução} %% TaskRunner, RunnableTask, RunnableTaskFactory,
            ...
            \textit{Reflection API}\footnote{\url{http://docs.oracle.com/javase/tutorial/reflect/index.html}} do Java. Finalmente, uma intância de \codechunk{RunnableTask} é construída e executada. Esse método é executado
            a cada dez segundos (por meio do plugin vraptor-tasks\footnote{\url{https://github.com/wpivotto/vraptor-tasks}}) e caso nenhuma tarefa esteja sendo executada, ela realiza o processo descrito acima.

        \subsection*{Cálculo das métricas de código} %% Metric, MetricFactory, CalculateAllMetricsTask (strategy + template method), CalculateMetricTask

    %% modelo de dados... destacar:  
    
    \section{Estendendo e contribuindo com o projeto}

    %% nova métrica, nova métrica pós processamento, novos scv, novas tasks em geral
