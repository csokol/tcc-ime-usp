...
\chapter{Avaliação da ferramenta} \label{ch:avaliacao}
    ...
    \section{Resultados obtidos com a mineração de repositórios públicos} \label{sc:apache}
        ...
        Para avaliar a capacidade da ferramenta desenvolvida, foram utilizados os projetos de código aberto disponibilizados pela \textit{Apache Software Foundation}. O código desses projetos pode ser acessado por meio do sistema de controle de versão Git, a lista completa de projetos se encontra em \url{http://git.apache.org/}. No total, foram cadastrados e processados 307 projetos de código aberto.
...
query.setParameter("project_id", project.getId())
    ...
    \section{Estudo de caso} \label{sc:estudo-caso}
        ...
        \begin{itemize}
            ...
            \item A maioria das refatorações descobertas realizavam apenas pequenas mudanças no código, como movimentação de métodos e variáveis entre as classes e não mudanças mais profundas como extração de classe, substituição de condicionais por polimorfismo, entre outros.
        \end{itemize}
