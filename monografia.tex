...
Nesta seção, a experiência obtida na realização do trabalho é relacionada ao curso de graduação no Bacharelado em Ciência da Computação.
    ...
    A seguir, a lista das disciplinas que contribuíram para este trabalho de conclusão:
    \begin{itemize}
        \item {\bf Princípios de Desenvolvimento de Algoritmos}: disciplina essencial para a implementação dos algoritmos envolvidos no desenvolvimento de qualquer sistema.
        \item {\bf Introdução aos Sistemas de Bancos de Dados}: disciplina essencial para a implementação dos algoritmos envolvidos no desenvolvimento de qualquer sistema.
        \item {\bf Laboratório de Programação Extrema}: primeiro contato com um sistema maior de código legado em Java. Essa experiência foi importante para encarar o desafio do desenvolvimento de um sistema maior.
        \item {\bf Engenharia de Software}: disciplina na qual entrei em contato com a Mineração de Repositórios além de desenvolver um projeto que envolvia tecnologias utilizadas nesse trabalho.
    \end{itemize}
